...
export default function Profile() {
  ...
  const infoRows = [
    ...
    { icon: IdCard,     label: 'User ID',    value: user?.userId || '—', mono: true },
  ]
          ...
          <div key={label} className="flex items-center gap-4 py-3 border-b border-slate-50 last:border-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {badge ? (
                ...
                <p className={clsx('text-sm font-semibold text-slate-800 mt-0.5', mono && 'font-mono text-xs break-all select-all')}>{value}</p>
              )}
            </div>
          </div>
        ))}
